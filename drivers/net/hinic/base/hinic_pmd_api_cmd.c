...
static void dump_api_chain_reg(struct hinic_api_cmd_chain *chain)
{
	...
	dev_err(dev, "chain type: 0x%x\n", chain->chain_type);
	dev_err(dev, "chain hw cpld error: 0x%x\n",
		HINIC_API_CMD_STATUS_GET(val, CPLD_ERR));
	dev_err(dev, "chain hw check error: 0x%x\n",
		HINIC_API_CMD_STATUS_GET(val, CHKSUM_ERR));
	dev_err(dev, "chain hw current fsm: 0x%x\n",
		HINIC_API_CMD_STATUS_GET(val, FSM));
	dev_err(dev, "chain hw current ci: 0x%x\n",
		HINIC_API_CMD_STATUS_GET(val, CONS_IDX));

	addr = HINIC_CSR_API_CMD_CHAIN_PI_ADDR(chain->chain_type);
	val  = hinic_hwif_read_reg(chain->hwdev->hwif, addr);
	dev_err(dev, "Chain hw current pi: 0x%x\n", val);
}
...
static int chain_busy(struct hinic_api_cmd_chain *chain)
{
	...
	case HINIC_API_CMD_PMD_WRITE_TO_MGMT:
		...
		if (chain->cons_idx == MASKED_IDX(chain, chain->prod_idx + 1)) {
			dev_err(dev, "API CMD chain %d is busy, cons_idx: %d, prod_idx: %d\n",
		...
		break;
	default:
		dev_err(NULL, "Unknown Chain type\n");
		return -EINVAL;
	}
				...
				void *cmd, u16 cmd_size)
{
	...
	case HINIC_API_CMD_PMD_WRITE_TO_MGMT:
		...
		break;
	default:
		dev_err(chain->hwdev->dev_hdl, "Unknown Chain type\n");
		return;
	}
...
static void api_cmd_status_update(struct hinic_api_cmd_chain *chain)
{
	...
	if (HINIC_API_CMD_STATUS_GET(buf_desc, CHKSUM_ERR)) {
		dev_err(dev, "API CMD status Xor check error\n");
		return;
	}
		       ...
		       __rte_unused void *ack, __rte_unused u16 ack_size)
{
	...
	case HINIC_API_CMD_PMD_WRITE_TO_MGMT:
		...
		if (err)
			dev_err(dev, "API CMD poll status timeout\n");
		break;
	case HINIC_API_CMD_WRITE_ASYNC_TO_MGMT_CPU:
		break;
	default:
...
static int api_cmd_chain_hw_init(struct hinic_api_cmd_chain *chain)
{
	...
	if (api_cmd_hw_restart(chain)) {
		pr_err("Restart api_cmd_hw failed\n");
		return -EBUSY;
	}
			 ...
			 struct hinic_api_cmd_cell *cell, u32 cell_idx)
{
	...
	if (!cmd_vaddr_alloc) {
		dev_err(dev, "Allocate API CMD dma memory failed\n");
		return -ENOMEM;
	}
			       ...
			       struct hinic_api_cmd_cell **node_vaddr)
{
	...
	if (!node_vaddr_alloc) {
		dev_err(dev, "Allocate dma API CMD cell failed\n");
		return -ENOMEM;
	}
	...
	/* Driver software should make sure that there is an empty API command cell at the end the chain */
	...
	case HINIC_API_CMD_PMD_WRITE_TO_MGMT:
		...
		if (err) {
			dev_err(dev, "Allocate cmd buffer failed\n");
...
static int api_cmd_create_cells(struct hinic_api_cmd_chain *chain)
{
	...
	for (cell_idx = 0; cell_idx < chain->num_cells; cell_idx++) {
		...
		if (err) {
			dev_err(dev, "Create API CMD cell failed\n");
			  ...
			  struct hinic_api_cmd_chain_attr *attr)
{
	...
	chain->cell_ctxt = (struct hinic_api_cmd_cell_ctxt *)
			   kzalloc(cell_ctxt_size, GFP_KERNEL);
	if (!chain->cell_ctxt) {
		pr_err("Allocate cell contexts for a chain failed\n");
		err = -ENOMEM;
	...
	chain->wb_status = (struct hinic_api_cmd_status *)
			   dma_zalloc_coherent(dev,sizeof(*chain->wb_status),
					       &chain->wb_status_paddr,
			   		       GFP_KERNEL);
	if (!chain->wb_status) {
		dev_err(dev, "Allocate DMA wb status failed\n");
		err = -ENOMEM;
				...
				struct hinic_api_cmd_chain_attr *attr)
{
	...
	chain = (struct hinic_api_cmd_chain *)kzalloc(sizeof(*chain),
						      GFP_KERNEL);
	if (!chain) {
		pr_err("Allocate memory for the chain failed\n");
		return -ENOMEM;
	}
	...
	if (err) {
		dev_err(hwdev->dev_hdl, "Initialize chain failed\n");
		goto chain_init_err;
	}

	err = api_cmd_create_cells(chain);
	if (err) {
		dev_err(hwdev->dev_hdl, "Create cells for API CMD chain failed\n");
		goto create_cells_err;
	}

	err = api_cmd_chain_hw_init(chain);
	if (err) {
		dev_err(hwdev->dev_hdl, "Initialize chain hw info failed\n");
