...
static void set_eq_ctrls(struct hinic_eq *eq)
{
	...
	if (type == HINIC_AEQ) {
		...
		(void)set_ceq_ctrl_reg(eq->hwdev, eq->q_id, ctrl0, ctrl1);
	}
}
 ...
 * Return: 0 - Success, Negative - failure
 **/
int hinic_aeqs_init(struct hinic_hwdev *hwdev, u16 num_aeqs, struct irq_info *msix_entries)
{
...
int hinic_handle_aeqe(void *handle, enum hinic_aeq_type event,
