 ...
 * @hwdev: the hardware interface of nic device
 **/
...
int hinic_init_attr_table(struct hinic_hwdev *hwdev)
{
	...
	rc = dma_attr_table_init(hwdev);
	HINIC_ERR_RET(hwdev->dev_hdl, HINIC_OK != rc, rc,
		      "Init dma_attr_table failed");
			   ...
			   void *buf_out, u16 *out_size, u32 timeout)
{
	...
	if (!hwdev || in_size > HINIC_MSG_TO_MGMT_MAX_LEN)
		return -EINVAL;
	/* If status is hot active, don't send message to mgmt */
 ...
 * @nic_dev: the hardware interface of a nic device
 **/
...
void hinic_show_ffm_info(struct hinic_hwdev *hwdev, void *buf_in, u16 in_size,
			 void *buf_out, u16 *out_size)
{
