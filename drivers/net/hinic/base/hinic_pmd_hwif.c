...
/* hinic internal functions declare */
static void hinic_parse_hwif_attr(hinic_nic_dev *nic_dev);
static int hinic_get_mmio(hinic_nic_dev *nic_dev, void **cfg_regs_base,
...
{
	/* io_mapping only for complie using hinic kernel, dwqe not support */
			  ...
			  void **intr_base, void **db_base)
{
	...
	pci_dev = nic_dev->hwdev->pcidev_hdl;
...
int hinic_hwif_res_init(hinic_nic_dev *nic_dev)
{
	...
	err = hinic_get_mmio(nic_dev, &cfg_regs_base, &intr_base, &db_base);
	HINIC_ERR_HANDLE(err != HINIC_OK, goto get_mmio_err,
			 "Get mapped mmio address failed, nic_dev: %s, error: %d",
			 err, nic_dev->proc_dev_name);
