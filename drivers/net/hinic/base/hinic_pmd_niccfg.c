...
int hinic_get_base_qpn(void *hwdev, u16 *global_qpn)
{
	...
	if (!hwdev || !global_qpn) {
		pr_err("Hwdev or global_qpn is NULL\n");
		return -EINVAL;
	}
...
int hinic_set_mac(void *hwdev, u8 *mac_addr, u16 vlan_id, u16 func_id)
{
	...
	if (!hwdev || !mac_addr) {
		pr_err("Hwdev or mac_addr is NULL\n");
		return -EINVAL;
	}
	...
	if (err || !out_size || (mac_info.mgmt_msg_head.status &&
		...
		dev_err(nic_hwdev->dev_hdl, "Failed to set MAC, err: %d, status: 0x%x, out size: 0x%x\n",
			err, mac_info.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
	if (mac_info.mgmt_msg_head.status == HINIC_PF_SET_VF_ALREADY) {
		dev_warn(nic_hwdev->dev_hdl, "PF has already set vf mac, Ignore set operation.\n");
		  ...
		  u16 func_id)
{
	...
	if (vlan_id >= VLAN_N_VID) {
		dev_err(nic_hwdev->dev_hdl, "Invalid VLAN number\n");
		return -EINVAL;
	}
	...
	if (err || !out_size || (mac_info.mgmt_msg_head.status &&
		mac_info.mgmt_msg_head.status != HINIC_PF_SET_VF_ALREADY)) {
		dev_err(nic_hwdev->dev_hdl, "Failed to delete MAC, err: %d, status: 0x%x, out size: 0x%x\n",
			err, mac_info.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
	if (mac_info.mgmt_msg_head.status == HINIC_PF_SET_VF_ALREADY) {
		dev_warn(nic_hwdev->dev_hdl, "PF has already set vf mac, Ignore delete operation.\n");
		return HINIC_PF_SET_VF_ALREADY;
	}
...
int hinic_get_default_mac(void *hwdev, u8 *mac_addr)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_MAC,
				     ...
				     &mac_info, &out_size);
	if (err || !out_size || mac_info.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to get mac, err: %d, status: 0x%x, out size: 0x%x\n",
			err, mac_info.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
...
int hinic_set_port_mtu(void *hwdev, u32 new_mtu)
{
	...
	if (err || !out_size || mtu_info.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set mtu, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_get_link_status(void *hwdev, u8 *link_state)
{
	...
	if (!hwdev || !link_state) {
		pr_err("Hwdev or link_state is NULL\n");
		return -EINVAL;
	}
	...
	if (err || !out_size || get_link.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to get link state, err: %d, status: 0x%x, out size: 0x%x\n",
...
/**
 * hinic_set_vport_enable - Notify fireware that driver is ready or not.
 * @hwdev: the hardware interface of a nic device
...
int hinic_set_vport_enable(void *hwdev, bool enable)
{
	...
	if (err || !out_size || en_state.mgmt_msg_head.status) {
		dev_err(hardware_dev->dev_hdl, "Failed to set vport state, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_set_port_enable(void *hwdev, bool enable)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_SET_PORT_ENABLE,
				     ...
				     &en_state, &out_size);
	if (err || !out_size || en_state.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set phy port state, err: %d, status: 0x%x, out size: 0x%x\n",
			err, en_state.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
...
int hinic_get_port_info(void *hwdev, struct nic_port_info *port_info)
{
	...
	if (!hwdev || !port_info) {
		pr_err("Hwdev or port_info is NULL\n");
		return -EINVAL;
	}
...
int hinic_set_pause_config(void *hwdev, struct nic_pause_config nic_pause)
{
	...
	if (err || !out_size || pause_info.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set pause info, err: %d, status: 0x%x, out size: 0x%x\n",
		      ...
		      u8 *pgid, u8 *up_bw, u8 *prio)
{
	...
	if (!hwdev || !up_tc || !pg_bw || !pgid || !up_bw || !prio) {
		pr_err("Hwdev, up_tc, pg_bw, pgid, up_bw or prio is NULL\n");
		return -EINVAL;
	}
...
int hinic_set_rss_type(void *hwdev, u32 tmpl_idx, struct nic_rss_type rss_type)
{
	...
	if (!cmd_buf) {
		dev_err(nic_hwdev->dev_hdl, "Failed to allocate cmd buf\n");
		return -ENOMEM;
	}
	...
	if (err || out_param != 0) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set rss context table\n");
		return -EFAULT;
	}
...
int hinic_get_rss_type(void *hwdev, u32 tmpl_idx, struct nic_rss_type *rss_type)
{
	...
	if (!hwdev || !rss_type) {
		pr_err("Hwdev or rss_type is NULL\n");
		return -EINVAL;
	}
...
int hinic_rss_get_template_tbl(void *hwdev, u32 tmpl_idx, u8 *temp)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_GET_RSS_TEMPLATE_TBL,
				     ...
				     &temp_key, &out_size);
	if (err || !out_size || temp_key.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to get hash key, err: %d, status: 0x%x, out size: 0x%x\n",
			err, temp_key.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
...
int hinic_rss_set_hash_engine(void *hwdev, u8 tmpl_idx, u8 type)
{
	...
	if (err || !out_size || hash_type.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to get hash engine, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_rss_set_indir_tbl(void *hwdev, u32 tmpl_idx, u32 *indir_table)
{
	...
	if (!hwdev || !indir_table) {
		pr_err("Hwdev or indir_table is NULL\n");
		return -EINVAL;
	}
	...
	err = hinic_cmdq_direct_resp(hwdev, HINIC_ACK_TYPE_CMDQ,
				     ...
				     cmd_buf, &out_param, 0);
	if (err || out_param != 0) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set rss indir table\n");
		err = -EFAULT;
		goto free_buf;
	}
...
int hinic_rss_get_indir_tbl(void *hwdev, u32 tmpl_idx, u32 *indir_table)
{
	...
	if (err || !out_size || rss_cfg.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to get indir table, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_rss_cfg(void *hwdev, u8 rss_en, u8 tmpl_idx, u8 tc_num, u8 *prio_tc)
{
	...
	if (!hwdev || !prio_tc || (tc_num & (tc_num - 1))) {
		pr_err("Hwdev or prio_tc is NULL, or tc_num: %u Not power of 2\n",
			tc_num);
		return -EINVAL;
	}
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_RSS_CFG,
				     ...
				     &out_size);
	if (err || !out_size || rss_cfg.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set rss cfg, err: %d, status: 0x%x, out size: 0x%x\n",
			err, rss_cfg.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
...
/**
 * hinic_rss_template_alloc - get rss template id from NIC,
...
int hinic_rss_template_alloc(void *hwdev, u8 *tmpl_idx)
{
	...
	if (!hwdev || !tmpl_idx) {
		pr_err("Hwdev or tmpl_idx is NULL\n");
		return -EINVAL;
	}
	...
	if (err || !out_size || template_mgmt.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to alloc rss template, err: %d, status: 0x%x, out size: 0x%x\n",
...
/**
 * hinic_rss_template_alloc - free rss template id to NIC,
 * all functions share 96 templates .
 * @hwdev: the hardware interface of a nic device
 * @tmpl_idx: index of rss template from NIC.
 * @return
...
int hinic_rss_template_free(void *hwdev, u8 tmpl_idx)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC_PORT_CMD_RSS_TEMP_MGR,
				     ...
				     &template_mgmt, &out_size);
	if (err || !out_size || template_mgmt.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to free rss template, err: %d, status: 0x%x, out size: 0x%x\n",
			err, template_mgmt.mgmt_msg_head.status, out_size);
		return -EINVAL;
	}
...
int hinic_set_rx_mode(void *hwdev, u32 enable)
{
	...
	if (err || !out_size || rx_mode_cfg.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set rx mode, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_set_tx_tso(void *hwdev, u8 tso_en)
{
	...
	if (err || !out_size || tso_cfg.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set tso, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_set_rx_lro(void *hwdev, u8 ipv4_en, u8 ipv6_en, u8 max_wqe_num)
{
	...
	if (err || !out_size || lro_cfg.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set lro offload, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_set_anti_attack(void *hwdev, bool enable)
{
	...
	if (err || !out_size || rate.mgmt_msg_head.status) {
		dev_err(nic_hwdev->dev_hdl, "can't %s port Anti-Attack rate limit, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_reset_port_link_cfg(void *hwdev)
{
	...
	if (err || !out_size || reset_cfg.mgmt_msg_head.status) {
		dev_err(dev->dev_hdl, "Reset port link configure failed, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_config_rx_mode(void *nic_dev, u32 rx_mode_ctrl)
{
	...
	if (err) {
		dev_err(nic_hwdev->dev_hdl, "Failed to set rx mode\n");
		return -EINVAL;
	}
...
void hinic_clear_vport_stats(struct hinic_hwdev *hwdev)
{
	...
	if (err || !out_size || clear_vport_stats.mgmt_msg_head.status) {
		dev_err(hwdev->dev_hdl, "Failed to clear vport statistics, err: %d, status: 0x%x, out size: 0x%x\n",
...
void hinic_clear_phy_port_stats(struct hinic_hwdev *hwdev)
{
	...
	if (err || !out_size || clear_phy_port_stats.mgmt_msg_head.status) {
		dev_err(hwdev->dev_hdl, "Failed to clear phy port statistics, err: %d, status: 0x%x, out size: 0x%x\n",
...
int hinic_flush_qp_res(void *hwdev)
{
	...
	if (err || !out_size || qp_res.mgmt_msg_head.status) {
		dev_err(dev->dev_hdl, "Failed to clear sq resources, err: %d, status: 0x%x, out size: 0x%x\n",
